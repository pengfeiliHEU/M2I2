...
from models.model_pretrain_noitc import M2I2
...
def train(model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler, config):
    ...
    for i, (image, text) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        # loss_mlm, loss_ita, loss_itm, loss_mim = model(image, text_input, alpha=alpha)
        loss_mlm, loss_itm, loss_mim = model(image, text_input, alpha=alpha)


        # loss = loss_mlm + loss_ita + loss_itm + loss_mim
        loss = loss_mlm + loss_itm + loss_mim
...
def main(args, config):
    ...
    datasets = [create_dataset('med_pretrain', config)]
    ...
    model = M2I2(config=config, text_encoder=args.text_encoder, tokenizer=tokenizer, init_deit=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='./configs/ICMC_pretrain.yaml')
    parser.add_argument('--checkpoint', default='')  # ./pre_weights/pretrain/ALBEF_4M.pth
